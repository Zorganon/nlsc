...
// TGMPA
require_once( get_template_directory() . '/includes/tgmpa/tgm-plugin-activation.php' );
	...
	class cc2_BootstrapButton {
		...
		public static function init() {
			...
			if ( get_theme_mod( 'fixed_top_nav', false ) != false ) {
				...
				add_filter( 'cc2_scroll_top_button_link', array(
					'cc2_BootstrapButton',
					'replace_top_link',
				) ); // replace top link
		...
		public static function add_bootstrap_button() {
			...
			$strScrollTopTemplate = apply_filters( 'cc2_scroll_top_button_template', '<span id="top-link-block" class="hidden top-link-container">' . '<a href="%s" class="well well-sm" id="top-link-button-link">' . '<i class="%s"></i><span class="top-link-button-text">%s</span>' . '</a>' . '</span><!-- /#top-link-block -->' );
 ...
 * TODO: Test if these could be tacked into the rest of the includes at the end of this file, or if there's a need to
 * load them all BEFORE the rest.
 */
 ...
 *
 * NOTE: Some plugins plus several modules for PHP (including eAccellerator) break the Closure support or do not
 * support them at all - DESPITE the correct PHP version 5.3+ - thus the original anonymous function call was replaced
 * with this outdated regular init function.
 */
...
function cc2_register_scripts() {
	...
	wp_register_script( 'cc-bootstrap-carousel', get_template_directory_uri() . '/includes/resources/bootstrap/js/carousel.js', array(
		'jquery',
		'cc-bootstrap-transition',
	) );
	wp_register_script( 'cc-bootstrap-collapse', get_template_directory_uri() . '/includes/resources/bootstrap/js/collapse.js', array(
		'jquery',
		'cc-bootstrap-transition',
	) );
	...
	wp_register_script( 'cc-bootstrap-popover', get_template_directory_uri() . '/includes/resources/bootstrap/js/popover.js', array(
		'jquery',
		'cc-bootstrap-tooltip',
	) );
	...
	wp_register_script( 'cc2-scroll-top', get_template_directory_uri() . '/includes/js/bootstrap-scroll-to-top.js', array(
		'cc-bootstrap-scrollspy',
		'cc-bootstrap-affix',
	), '2.0', true ); // run in footer - else the document isnt rendered fully, and thus, the window hieght check will fail
	...
	if ( ! empty( $advanced_settings['headjs_url'] ) ) {
		$headjs_url = str_replace( array(
			'%template_directory_uri%',
			'%stylesheet_directory_uri%',
		), array( get_template_directory_uri(), get_stylesheet_directory_uri() ), $advanced_settings['headjs_url'] );
	}
