...
class Events_Maker_Admin {
	...
	public function admin_scripts_styles( $pagenow ) {
		...
		if ( $pagenow === 'edit-tags.php' && in_array( $post_type, apply_filters( 'em_event_post_type', array( 'event' ) ) ) ) {
			// event location & organizer
			if ( in_array( $taxonomy, array( 'event-organizer',  'event-location', 'event-category' ) ) ) {
