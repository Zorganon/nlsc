...
<a href="http://smartypantsplugins.com/sp-client-document-manager/" target="_blank" class="button">Click here to upgrade! </a> <a style="margin-left:10px" href="http://www.youtube.com/watch?feature=player_embedded&v=m6szdA3r-1Q" target="_blank" class="button">View the youtube video</a> <a style="margin-left:10px" href="http://smartypantsplugins.com/donate/" target="_blank" class="button">Click here to donate</a> <a href="admin.php?page=sp-client-document-manager&sphidemessage=1"  class="button" style="margin-left:10px">Click here to ignore us!</a></p>
	...
	if (@$_GET['ignore'] == 'shortcode') {
		...
		if($r[0]['ID'] == ""  && get_option('cdm_ignore_shortcode') != 1){
			...
			if ( ! isset( $_REQUEST['cdm_nonce'] ) || ! wp_verify_nonce( $_REQUEST['cdm_nonce'], 'cdm_nonce' )) {exit('Security Error');}	
				// Create post object
			$my_post = array(
			  'post_title'    => sanitize_text_field($_POST['page-name-cdm']),
			...
			$content .= '<div style="margin:10px;font-size:1.3em" class="sp_cdm_success"><strong>'.sanitize_text_field($_POST['page-name-cdm']).'</strong> Page Created! <a href="'.get_page_link($post).'" target="_blank">Click here to preview the page</a></div>';	
				}else{
    ...
    if (!function_exists('sp_client_upload_admin')) {
        function sp_client_upload_admin()
        {
            ...
            $file = sanitize_text_field($_GET['dlg-delete-file']);
		    if (  $file  != "") {
                $r = $wpdb->get_results("SELECT *  FROM " . $wpdb->prefix . "sp_cu   where  id = " .   $file  . "", ARRAY_A);
                @unlink('' . SP_CDM_UPLOADS_DIR . '' . $r[0]['uid']. '/' . $r[0]['file'] . '');
                $wpdb->query("

	DELETE FROM " . $wpdb->prefix . "sp_cu WHERE id = " .   $file  . "

	");
            }
			if($_POST['cdm_limit'] != ''){
			update_option('cdm_admin_listings_limit' , sanitize_text_field($_POST['cdm_limit']));
			}
			$limit = get_option('cdm_admin_listings_limit', 15);
			
			if($_POST['from'] != '' ){
			$search .=	" AND date > '".date("Y-m-d",strtotime($_POST['from']))."' ";
			}
			if($_POST['to'] != '' ){
			$search .=	" AND date < '".date("Y-m-d",strtotime($_POST['to']))."' ";
			}
            if ($user_id != "") {
			...
			if($_GET['page'] == 'sp-client-document-manager'){
			$html .='<script>
  jQuery(function($) {
    $( "#from" ).datepicker({
      defaultDate: "+1w",
